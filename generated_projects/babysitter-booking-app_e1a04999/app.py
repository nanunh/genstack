...
This application is a web platform that connects parents with babysitters. 
It allows parents to browse available sitters, book their services, and leave reviews.
Sitters can create profiles, manage their availability, and accept or reject booking requests.

Key features:
- User registration and authentication (parents and sitters)
- Sitter profile management
- Booking system with request/accept/reject workflow
- Review and rating system
- Dashboard views for both parents and sitters
...
class User(UserMixin, db.Model):
    """User model representing both parents and sitters in the system."""
...
class SitterProfile(db.Model):
    """Profile information specific to babysitters."""
    id = db.Column(db.Integer, primary_key=True)
...
class Booking(db.Model):
    """Represents a babysitting booking between a parent and a sitter."""
    id = db.Column(db.Integer, primary_key=True)
...
class Review(db.Model):
    """Reviews left by parents for sitters after completed bookings."""
    id = db.Column(db.Integer, primary_key=True)
...
def register():
    """User registration for both parents and sitters."""
...
def login():
    """User login with redirection to appropriate dashboard."""
    if request.method == 'POST':
        email = request.form.get('email')
        password = request.form.get('password')
...
def parent_dashboard():
    """Dashboard for parents showing available sitters and booking history."""
...
def sitter_dashboard():
    """Dashboard for sitters showing their profile and booking requests."""
...
@login_required
def sitter_profile():
    """Allows sitters to create and update their profile information."""
    if current_user.user_type != 'sitter':
        return redirect(url_for('index'))
...
def view_sitter(sitter_id):
    """View a sitter's profile and reviews."""
...
def book_sitter(sitter_id):
    """Allows parents to book a sitter for a specific time period."""
    if current_user.user_type != 'parent':
        return redirect(url_for('index'))
...
def update_booking(booking_id):
    """Allows sitters to accept or reject booking requests."""
...
def review_booking(booking_id):
    """Allows parents to leave reviews for sitters after completed bookings."""
    booking = Booking.query.get_or_404(booking_id)
